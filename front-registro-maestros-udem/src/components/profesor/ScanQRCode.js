...
const ScanQRCode = () => {
	...
	useEffect(() => {
		...
		if (session) {
			...
			fetch("http://172.32.149.244:5096/QR/GetCourseData/" + nomina)
		...
		fetch("http://172.32.149.244:5096/QR/GetCourseData/" + nomina)
			...
			.then(data => {
				...
				if (data !== -1) {
					...
					const QRCode = ({ nomina }) => (
						<QRCodeSVG value={`http://172.32.149.244:3000/profesor/qr/${nomina}/` + (type === 1 ? `1` : `2`) + `/` + encrypted} size={250} />
					);
					...
					document.getElementById("qrLink").innerHTML = "O haga clic <a href='" + ("http://172.32.149.244:3000/profesor/qr/" + nomina + "/" + (type === 1 ? "1" : "2") + "/" + encrypted) + "' target='_blank'>aquí</a>";
				}
